...
export function ParentChildrenWorkspace() {
  ...
  const hasAcademicSummary = Boolean(
    selectedDetail &&
      (selectedDetail.average !== null ||
        selectedDetail.class_rank !== null ||
        selectedDetail.grade_rank !== null),
  );
  const hasAcademicDetails = Boolean(
    selectedDetail &&
      (selectedDetail.teachers.length ||
        selectedDetail.attendance ||
        selectedDetail.latest_grades.length),
  );
  const hasSupportingDetails = Boolean(
    selectedDetail &&
      (selectedDetail.quick_links.length ||
        ...
        <article className="panel-card">
          {detailLoading ? <PanelLoading label="در حال دریافت پرونده تحصیلی..." /> : detailError ? <PanelError message={detailError} onRetry={() => setDetailVersion((value) => value + 1)} /> : detail && String(detail.id) === String(effectiveSelectedId) ? (
            <>
              ...
              {hasAcademicSummary ? <div className="mt-5 grid grid-cols-3 divide-x divide-x-reverse divide-slate-200 rounded-lg border border-slate-200 p-4 text-center">
      ...
      {detail && String(detail.id) === String(effectiveSelectedId) ? (
        <>
          {hasAcademicDetails ? <section className="grid gap-5 xl:grid-cols-3">
            ...
            {detail.latest_grades.length ? <article className="panel-card">
              ...
              <div className="overflow-hidden rounded-lg border border-slate-200"><table className="panel-table"><thead><tr><th>درس</th><th>نمره</th><th>وضعیت</th></tr></thead><tbody>{detail.latest_grades.map((grade) => <tr key={grade.id}><td className="font-black text-[#172b43]">{grade.subject}</td><td>{grade.score}</td><td>{grade.status_label ?? "—"}</td></tr>)}</tbody></table></div>
            </article> : null}
          </section> : null}

          {hasSupportingDetails ? <section className="grid gap-5 xl:grid-cols-[1.1fr_1fr_1fr]">
            {detail.quick_links.length ? <article className="panel-card">
            ...
            {detail.exams.length ? <article className="panel-card">
              ...
              <div className="space-y-3 text-xs font-bold text-slate-600">{detail.exams.map((exam) => <div key={exam.id} className="flex justify-between gap-3 border-b border-slate-100 pb-2"><span>{exam.title}</span><time>{formatDate(exam.starts_at)}</time></div>)}</div>
            </article> : null}
          </section> : null}
        </>
      ) : null}
